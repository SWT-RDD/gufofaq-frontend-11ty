...
//
// 為什麼鎖不寫成 js 的共享計數器：跳窗與手機選單是兩個互不知情的擁有者，各鎖各的話，
// 先關的那一個會把還開著的那一個一起解鎖，而要修就得引進一顆全域計數器與它的失衡風險。
// `:has()` 是宣告式的 OR —— 狀態就在 DOM 上，數不可能對不起來，巢狀開窗、resize 自動收合、
// Esc 關窗全部自動成立，一行 js 都不必寫。
//
