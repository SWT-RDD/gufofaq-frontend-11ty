...
//
// 為什麼是切版的事（§5 ④，round44 裁決）：真 app 的排序也是純前端——凍結前端
// `GufoFAQ_Frontend_New/js/previewExcelCompare.js` 的 `$(document).on('click','.sort')` 走
// `sortCompareData()`／`sortOriginalData()`，兩支都只是對已在記憶體的陣列做 `.sort()`、沒有 fetch；
// `qaHistory.js` 的同名委派對 `currentChatLogs` 就地重排，同理。
...
// round44 之前全站 15 顆 `.sort` 一顆都不動，是 §5「不放按了沒反應的鈕」的違規。
//
// 行為規格（轉 React 時這就是規格）：
...
//      不該被當成欄排序鈕）。欄索引是那顆 `<th>` 在 `<thead> <tr>` 裡的位置；`data-column` 是真 app
//      的既有掛點（`previewExcelCompare.js` 的 `.sort` 委派），本元件不讀它。
