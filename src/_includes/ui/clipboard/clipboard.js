...
//   `components/import-report`（來源是組出來的出口替換規則）。**寫剪貼簿這件事全站只有這一支**
//   ——同樣的東西有兩個呼叫端就抽出來，抽的單位是重複的那一段（§3-3）；呼叫端各自留一份
//   `execCommand` 退路的話，兩份會分岔，而分岔的那一半只在 `navigator.clipboard` 不可用時才跑到。
//
// ── 宣告式用法：`.shareBtn` ────────────────────────────────────────────────────
