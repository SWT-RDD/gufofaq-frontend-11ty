...
// 所以本檔**沒有**延後關窗的 300ms setTimeout、**沒有** `.show`/`.hide` 狀態 class、也**不需要**
// 「關到一半又點關閉／關到一半又重開」的重入守衛：`close()` 立刻拿掉 `[open]`，瀏覽器自己把元素
// 撐到退場動畫跑完才真的 `display:none`；中途再 `showModal()` 的話 transition 原生就會反向。
// 這三樣東西只要補回任何一樣，就等於在 CSS 之外多開一份「窗現在是開是關」的真相。
