...
document.addEventListener("DOMContentLoaded", function () {
    window.GufoSources = {
        ...
        // 展開該列、捲到畫面中央、短暫高亮。
        // **找不到該筆就什麼都不做（連區塊都不掀）**：先 show() 再找列的話，按一個表上沒有的號碼
        // 會掀開一張沒有任何列高亮的表，讀的人會以為那一筆就在裡面、只是自己沒看到——那比什麼都
        // 不發生更難查。這道守衛擋的是**號碼落在這張表的範圍之外**。
        // ⚠️ **它擋不住「號碼落在範圍內、但那是另一側的表」**：同頁只放一份表時（2-2-3 就是這個
        // 形狀——一份 A 側的表，而 B 側答案也有自己的引用號），另一側的號碼會打到這一份而且真的
        // 高亮得起來。那不是這裡收得掉的，換側時整份表要一起換（`sourcesSideKey` 就是那個宣告），
        // 兩側各自的來源表是業務 js 的事。
        reveal: function (no) {
