...
//
// 為什麼這段行為在切版就要動得起來（§5 ④）：它在正本也是純 UI state（saas skills/page.tsx 的
// `setTryTarget(skill)` / `setTryTarget(null)`），沒有 API、不需要業務資料——名字就在被點的那一列上。
// 「區塊的顯示條件是業務的」不能推導出「區塊內的鈕也是業務的」，否則整區三顆鈕都變成沒人接的死鈕。
//
// 觸發鈕的 class 沿用 `.js-try-skill`：它已被 saas（skills/page.tsx 與 e2e/skills.spec.ts）引用而凍結，
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // 委派掛在 document 上：清單列是頁面層的 markup（本元件不擁有它），且真實環境的列由 React 重繪。
