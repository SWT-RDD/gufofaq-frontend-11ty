...
test("§4 每一顆會改狀態的鈕都要宣告它需要哪一道閘門（四條授權軸，值＝§4 宣告的那四組值域）", () => {
    ...
    const NO_GATE = new Map([
        ...
        ["src/_includes/ui/toast/toast.html", new Map([
            ["已成功儲存", "型別展示片段（只被元件庫頁 include）：這四顆鈕示範的是 toast 的四種長相，按下去只在本機彈一則字、不打任何端點——授權軸講的是「送出這件事需要什麼權限」，而這裡沒有那件事"],
        ])],
        ["src/login.html", new Map([
            ["登入成功", "登入是**認證之前**的那一顆：這時還沒有主體，能力／租戶管理員／平台角色都是登入之後才判得出來的東西，宣告不出任何一道閘門。它也不是「唯讀所以免標」——登入會寫 session、會落稽核"],
        ])],
    ]);
