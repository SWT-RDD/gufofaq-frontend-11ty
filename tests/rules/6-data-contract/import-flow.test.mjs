...
test("§6 1-2-1 批次匯入：索引同步逐檔一顆徽章，匯入失敗那一列畫缺席態、不畫「寫入索引失敗」", () => {
    // 索引同步的狀態是**逐檔**一份（一檔一條各自的索引管道、各自成敗），
    // 壓成一顆彙總徽章之後「三檔還在同步、一檔查不到結果」會整塊變成查不到——另外那幾檔
    // 在畫面上消失了。這條測試釘三件事：
    //   ① 每一列都有自己的那一顆（漏一列＝那一檔的狀態又不見了）
    //   ② **匯入失敗**的那一列畫**缺席態**（`.is-faint`「沒有索引任務」）：那一檔連一條索引管道
    //      都沒有（檔沒進去，就沒有任何任務可查），給它一顆「寫入索引失敗」是在講一件
    ...
    const rule = (tbody) => {
        ...
        for (const row of rows) {
            ...
            if (failed) {
                if (key !== ABSENT) out.push(`${who}：匯入失敗那一列的同步格畫成 ${key}——那一檔連一條索引管道都沒有，應該是缺席態 ${ABSENT}`);
...
test("§6 1-2-1 批次匯入：彙總的 importSyncState ＝逐檔優先序取最安全的那一邊", () => {
    ...
    // 製造重複資料的那一邊。React 端從逐檔那一份結果現算；切版這一側因為 nunjucks 的
