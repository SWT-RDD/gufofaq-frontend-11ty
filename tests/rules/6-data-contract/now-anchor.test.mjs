...
// 日期字面**三種形狀**：`2026-07-14`／`2026/07/14`（兩種分隔符全站都有）、中文的
// `2024年12月1日`（欄位型別示範的髒資料就長這樣），以及落地檔名前綴那顆 14 位時間戳
// （`20260714105623_…`）。只認前一種的話，後兩種畫得出時間的頁面整批在網外——而它們正是
// 最容易漂的那一族：中文日期是刻意的髒資料樣本，時間戳長在「剛匯完」那一頁上，兩種都會
// 靜靜地停在一個與站台的「現在」對不上的時刻。
const DATE = /\b20\d\d[-/]\d{1,2}[-/]\d{1,2}\b|20\d\d年\d{1,2}月\d{1,2}日|(?<!\d)20\d{12}(?!\d)/g;
