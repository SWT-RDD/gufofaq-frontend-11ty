...
export const NAMED_HOOKS = new Map([
    ...
    ["singleSelect", "原生單選 <select> 的標記：與 .multiSelect／.searchSelect 三選一，標了它就是「這一顆維持原生下拉、不加強化」。全站沒有任何 js 查它（多選由 ui/multi-select 查 .multiSelect、搜尋型由 ui/search-select 查 select.searchSelect），它的消費者是 React 那一側的元件選型（gufofaq-saas apps/web 逐處照抄同一組 class）"],
