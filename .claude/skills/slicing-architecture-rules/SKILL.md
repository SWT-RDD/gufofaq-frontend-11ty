...
- 新增一種「用途 × token」的組合時先問：**這條路徑有沒有測試覆蓋？chrome 桶是不是讓它免檢查了？** 對比測試把 token 分成填充／文字／前景墨色／表面／chrome 五桶，chrome 桶完全豁免、填充桶不在「文字族當底色」的掃描範圍——把它們挪去當需要檢查的角色，錯誤就隱形——例如拿 chrome 桶的 `--border` 當遮罩墨色，深色下可能掉到 1.3:1 而沒有任何一條測試會紅。沒覆蓋就補一條掃編譯後 css 的測試。
