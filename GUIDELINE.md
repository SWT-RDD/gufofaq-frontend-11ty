...
- html / scss / js 三種檔案**有才放**：純樣式元件只有 scss（`ui/ab-test-block`）、純行為元件只有 js + scss（`ui/modals`）。**無 html 的元件，它的 markup 契約逐字寫在該元件 scss（或 js）的檔頭**；**展示片段（html 只被元件庫頁 include）同辦**——它們的生產 markup 逐字散在使用頁上，同樣是「必然被複製、少一顆屬性視覺指紋看不出來」的那一族，故只要生產實例帶了片段沒有的硬規則屬性，生產契約就要寫在該元件的 scss／js 檔頭（這條已機器化），並在 README 的「無 html 元件」清單登記——否則「這顆東西的 markup 長什麼樣、住在哪一頁」全站無處可查（§6 說參數正本在 html 檔頭，無 html 就無處可寫）。契約含 hook class、`data-*`、`.sr-only`＋i18n 屬性的完整組合：這種元件的實例必然被複製，而少掉一個屬性視覺指紋看不出來
  - **契約的形狀是「一段可以整段照抄的 markup」**（帶 `<` 的真標籤），不是散文列幾個 class 名。判準：下一個人照著檔頭寫出來的東西，要能與現有實例逐字相同。散文寫得再詳細，抄的人也還是得去翻某一頁的原始碼——而那正是這條規則要消滅的動作。
    ...
    - **契約要與實例對得上，而不是與記憶對得上**：新增第二個消費點時回頭重寫一次，逐字比對至少一個現有實例（class 串、巢狀層數、屬性）。差一層祖先或少一顆屬性都算沒對上。
      ...
      - **契約段裡合法的省略只有兩種，都要寫成不會被複製進 HTML 的形狀**：①重複的同型節點（`{# 重複 N 次同型節點 #}`）；②**明確屬於另一個元件、而且該元件檔頭有完整契約的相鄰段落**（`{# 此處接 ui/modals 的 .modals-footer，見該元件檔頭 #}`——從 `.modals-wrap` 寫起的契約必然要略掉殼的其餘部分，那不是省略、是分工）。**除這兩種之外，契約段內不准有任何 `{# #}`**：用它描述一顆被略掉的節點該有哪些屬性（`{# 逐顆 button-icon：各自帶 id ＋ aria-labelledby #}`），形式上不是散文、也不會被貼進 HTML，於是兩張網都碰不到，而被略掉的恰好是硬規則屬性。用中文括號句放在段落中間，形式上沒有 `…`、實質是省略，而整段複製會把散文貼進 HTML——差別要看得出來是「形狀」而不是「語氣」。
...
- **同一條多步驟流程的各步驟頁，`titleKey` 必須可分辨**：沿用同一顆 key 的規則管的是「同名的不同頁面」，不是「同一頁面的不同步驟」——七頁的 `<title>` 與 sr-only h1 逐字相同時，報讀器使用者與瀏覽歷史都分不出自己在第幾步。
...
- **註解對上游行為的斷言，寫時要對過正本並附出處。出處＝ repo 名 ＋ 符號名（＋ HTTP 端點），三者之外的一律不寫**——特別是**上游的內部模組路徑與檔名一個字都不准出現**（有測試把關）。理由不是簡潔：那一欄是整條引用裡**唯一**會因為對方重構而死掉的部分，而本專案的測試比不到隔壁 repo 的 Python，它壞掉沒有任何一關會紅。實測（上游兩次改組之後）：637 處跨 repo 引用有 149 處指到不存在的檔，其中 **148 處符號還在、值還對**——訊噪比 1:148，而唯一真正過期的那一條被埋在噪音裡。**符號名反過來活得下來**：檔案搬家、函式不改名；改名了就是一個**該被看見**的訊號。同一條的既有推論在低一層已經下過（「不得引行號」——行號會漂到語意相反的那一支），只是把它往上推一層：帶目錄的檔名有一模一樣的性質。
  ...
  - **可引的正本只有一級**：gufofaq-saas（services/product、apps/web、docs）＋ GufoRAG（chatbot）。**這一級以外的任何前端／後端都不是正本、也不是考據來源**，註解裡不引：切版是唯一真實來源，一個設計成立與否不靠「別的地方也這樣做」背書。
  - **路徑禁令寫成白名單，不是黑名單**：註解裡不准出現**任何帶副檔名的上游檔名或帶目錄的模組路徑**，也不准 `<lower_snake>.<SYMBOL>` 這種模組限定詞（`health_checks.NEAR_DUPLICATE_MAX`／`chat_config_limits.MCP_FIELD_LIMITS`）——只留 repo 名 ＋ 符號名。**唯一例外是我們自己的下游**：gufofaq-saas 的 `apps/web/**` 與 `docs/**`（副檔名不限，含 `.json`／`.md`／`public/*.js`），引它要帶 repo 名。理由：黑名單漏一種副檔名，那一族就永遠判不成違規（`\.py` 那一條擋不到 `.ts` 以外的任何東西），而**不算模組限定詞的合法形狀**是類別屬性（`Settings.stats_max_logs`）、型別欄位（`FindingOut.records`）與常數子鍵（`DATASET_LIMIT_GROUPS.alias`）。
...
- **會改狀態的鈕要宣告它需要哪一道閘門**（有測試把關）。四條授權軸各一個屬性，值一律是**上游閘門自己的名字**、不另發明詞彙：`data-capability="data:write"／"settings:write"…`（對 `require_capability(...)`，值域＝product `CAPABILITY_TOKENS`）、`data-tenant-feature="ask"／"data"…`（對 `require_tenant_feature(...)`，值域＝product `CAPABILITIES`）、`data-tenant-role="admin"`（對 `require_admin`——租戶管理員旗標不是一顆能力，故另立一軸）、`data-platform-role="admin"／"auditor"`（對 `require_platform_admin／_auditor`）。
  ...
  - **`data-tenant-feature` 的判準是端點上有沒有 `require_tenant_feature`，與呼叫方有沒有登入態無關**：公開端點（widget token／分享連結）照樣標得出來、也照樣要標——那道閘判的是租戶不是主體，而 widget token 認得出租戶。「匿名訪客身上沒有能力或角色可以判」只免除 `data-capability`／`data-tenant-role`／`data-platform-role` 三軸。
  ...
  - **`data-capability` 不足時**：**不比照 `data-platform-role` 做「不渲染」，而是照 §5「送 API 的鈕列全結果」把 403 列進 `data-toast`**。理由是兩軸的粒度不同：平台角色的單位是「整塊唯讀」，React 端關掉一整塊做得到；能力 token 是逐顆的細粒度（一頁可能有二十顆鈕各要不同 token），逐鈕過濾在應用層做不出來，而「有 `settings:read` 沒 `settings:write`」的人打得開頁面、看得到鈕 ⇒ **那道 403 是真實可達的結果路徑**。子句全站統一（「權限不足，無法<動詞>——請找貴租戶的管理者開通」這一族），語意型別一律 warning。**收件人是子句的一部分，不是可選的裝飾**：`require_capability` 第④層那道 403 帶 `code: missing_capability`，而它只在「這個人少了能力 token」時發生（平台管理員 bypass、租戶管理者 `is_admin` 直接過）——所以收件人**全站唯一且確定**＝貴租戶的管理者。只寫「權限不足」的話，使用者知道被擋、卻不知道這是不是一個修得掉的狀態，也不知道要找誰；那正是這一族最常被回報的問題。**但第③層那道不同**（`require_tenant_feature`：平台把整個租戶的某個功能關掉，連租戶管理者也擋）——它的收件人是平台管理員，故要**另開一段**、不可以與能力那一段共用一句話（§4「兩組不同的鍵…失敗的方式也不同」）。**判準是「那道 403 走不走得到」，不是「標了哪一軸」**：`data-capability` 一掛，`tenant_feature_disabled` 就是真實可達的結果路徑（`require_capability` 內部串著它），所以 `data-toast` **末位一律追加**「此功能尚未對貴租戶開通，請聯絡平台管理員」（warning）；`data-tenant-feature` 只在「租戶開通鍵與 `data-capability` 自己的領域不同」時另標一軸，與那一段的有無無關。**閘不是 `require_capability(...)` 的那一族更要自己一段**：自訂複合閘（`require_audit_access` 這種自己直接呼叫 `require_active_subscription`／`require_tenant_feature` 的）身上的 `data-capability` 只是最窄那一條的近似值——**逐 `raise` 對回鈕，不是逐「軸」對回鈕**。
  ...
  - **查證過確定不需要閘門的，要留下痕跡**（頁檔頭寫明「這支是自助端點，只要登入態」）：空白與「忘了標」在 markup 上長得一模一樣。**痕跡要成對**：測試白名單登記一筆，就要在對應的元件／頁檔頭寫同一句理由——只住在測試裡的豁免，讀元件的人看不到。**判準寫成可跑的**：每一筆豁免的值除了理由，再帶一個**檔頭錨點字串**（該段註解的開頭幾個字，同 §8-1 第 5 條「指得到一個 grep 得回去的東西」，不寫行號），測試斷言那支檔真的含那串字。這條對 `POSITIONAL`／`EXEMPT`／`EMPTY_EN_ALLOWED` 那幾張表同樣適用。**判準是「打了掛 `require_*` 的端點」，不是「會不會改狀態」**：讀取型端點（`require_capability("settings","read")`）同樣要標或寫明查證結論。**宣告錯一道比沒宣告更貴**——`/me/profile` 標成 `settings:write`，React 就會把「改自己的顯示名」擋在一顆它根本不需要的能力後面。
...
- **同繁中且英譯逐字相同的兩顆 key 一律不放行，白名單也不例外**：那代表兩顆 key 連怎麼說都沒有分岔，也就沒有分成兩顆的理由（同頁並存時最刺眼）。白名單只收得下「英譯確實不同」的那一種（`Members`／`Member count`）；**理由要逐組寫，不准一句話涵蓋七個字**——通用理由等於預先放行下一次的另立。
...
- **英譯字串裡不得出現全形標點**（`「」`、`＝`、`：`、``…，有測試把關）：那是繁中的字身，混在英文句子裡會露出一個明顯不屬於這個語言的符號。改用 `“ ”`、`=`、`:`、`( )`。唯一例外是「被引用的樣本字面」（在講一個字面上就是全形的東西時）
  - **繁中原文的標點字身也只有一種拼法**：句末不帶驚嘆號（`已複製!`／`已複製！`／`已複製` 三種並存時，同一顆 toast 元件會彈出三種字身）、省略號一律 `…`（U+2026，不用三個半形點）。不擋的話「不是全形」就永遠是合規的下限，而那正是英譯那一條的同一個理由。
...
- **列序也是示範資料的一部分**：表格的示範列順序必須是那支端點的 `order_by` 產得出來的順序，排序出處寫在 `{% set %}` 上方，形狀兩種擇一——①`<repo 名> 的 <符號名>` 的 `order_by`（**不寫檔名**，同 §3-2）；②**上游不宣告順序**（代理式端點原樣轉出上游回應）——寫明這一句，並說出示範列照什麼排、為什麼那個排法是上游可能產出的。空著不算查證過。照敘事順手排列＝演一個 API 產不出來的畫面，而它讀起來完全合理（`platform_role.desc()` 會把 auditor 排在 admin 前面、`created_at.desc()` 會把 id 3 排在 1 前面——兩者都與「照 id 升冪寫下去」相反）。
- **列鍵是資料契約的一部分**：凡渲染**可刪除**清單的元件，其參數陣列必須帶身分欄位（`id`／`sn`），markup 用它組列鍵與逐列 id；`loop.index` 只准用在成員固定的清單。位置不是身分——刪一筆之後每一顆鍵整排前頂，而 React 的 key 與逐列動作會一起指錯人。有測試把關（判準放寬到「列內任何地方」：列鍵掛在動作控制項上而不是 `<tr>` 也算數）。
  - **位置真的就是身分時，要寫出「為什麼上游沒有 id」**，而理由的值域是**閉合的三種**：整批取代的編輯器（`PUT …/entries`，docstring 明講「不做逐筆 diff」）、`Column(JSON, default=list)` 的 `list[dict]`、還沒落庫的新增表單——這三種的上游正本本來就是一個沒有身分欄的陣列。豁免寫進測試的 `POSITIONAL` 表、附上游符號名；空著的理由不算查證過。**「這一頁的 React 用閉包取值」「刪除鈕不印列鍵」不是理由**——那描述的是消費端怎麼寫，不是上游有沒有 id；**同一張上游表已經有另一頁用 `id` 當列鍵時，這筆豁免自動失效**。另外，**列鍵的值不得是 `loop.index*`**：`data-index="{{ loop.index0 }}"` 會滿足「屬性名裡有 index」這種形狀判準而整條規則從未對它執行過——屬性名裡有 `index` 不等於裡面裝的是身分。
...
- **開關／枚舉的「另一態」要真的有一頁渲染得出來，預設態不算**：內建工具**全開**＝上游 `enabled_builtin_tools` 的 `None`，那顆開關的「關」於是全站沒有一頁畫得出來。挑一顆與其他推導無關的關掉，同時也把「白名單真的在用」演出來（清單是一串名字，不是 `None`）。（顆數不寫死，§3-2。）
