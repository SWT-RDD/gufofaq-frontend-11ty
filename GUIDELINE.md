...
下面五條是這個 repo 實際被咬過的五種塌法，每一條都要在寫新規則的同時一起寫下去。
...
2. **棘輪要跟著母體一起長**：`assert.ok(total >= N)` 的 `N` 只有在它是**這次實際量出來的**時候才綁得住；沿用一個舊數字＝那條守門形同不存在。
   母體長了三千個標籤而門檻沒動，等於留了 15% 的縫（剝除規則吃掉一成的真 markup 仍會全綠）。
...
4. **白名單不可以是萬用前綴**：`^is-[a-z0-9-]+$` 這種寫法是把整個前綴讓出去——`is-completd` 這種錯字、
   ...
   `ui/field-slot-catalog` 的 22 槽推出；沒有樣式的狀態契約逐筆列名寫理由），這樣目錄少一項、
   markup 打錯一個字，測試就會當場報出來。

5. **「這是上游的掛點」是一句可以查證的斷言**：具名 hook 的白名單要存成 `名字 → 出處`（檔＋行／符號名），
   並由測試釘住「每一筆都寫得出出處」。查不到出處的豁免與憑空放行沒有分別——實測改成這個形狀之前，
   有 24 筆的出處在全站註解裡一個字都找不到。
