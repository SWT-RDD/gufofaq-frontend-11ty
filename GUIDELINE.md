...
- **會改狀態的鈕要宣告它需要哪一道閘門**（有測試把關）。四條授權軸各一個屬性，四軸的值域**在這裡宣告一次**、各自封閉，不另發明詞彙：`data-capability="data:write"／"settings:write"…`（能力 token，形狀是 `<資源>:<read|write>`）、`data-tenant-feature="ask"／"data"…`（租戶層的功能開通）、`data-tenant-role="admin"`（租戶管理員旗標不是一顆能力，故另立一軸）、`data-platform-role="admin"／"auditor"`（平台角色）。
  ...
  - **四軸之外的「資源層 scope」不是閘門，是結果分支。** 同一支端點常在能力閘門之後另有一道「他可以寫**哪一個**資料集／哪一個群組」的守衛（那一道 403）——四軸表達不出它，於是它在 markup 上完全隱形。它的家在 §5 的 `data-toast`：列成一段 warning（使用者找得到人開通／換群組），不要折進 error。判準：**清單端點不依該 scope 過濾**（看得到一筆、按下去 403）就是真實可達，必須列。
...
- **只翻 UI chrome，不翻假資料**：聊天訊息、提示詞、免責聲明內文、示範檔名／資料集名、表格 cell 值、示範 Excel 欄位一律不翻。
  - **「租戶自己取的名字」一律是資料，不是 chrome**：欄位槽的**顯示名**（`document_field_mapping` 那一份）、群組名、資料集名、標籤維度名都屬這一族——掛了 `data-i18n` 就會把使用者自己命名的欄改寫成英文。**產品的預設名才是 chrome**（`ui/field-slot-catalog` 的 `defaultLabel` → `field.<key>`，那是產品自己的預設名、與租戶無關）。判準一句話：**那個字串是誰打的？租戶打的就是資料。**
