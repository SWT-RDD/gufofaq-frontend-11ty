...
- **分隔空白的家在英譯，不在繁中原文**：前後綴 key 的繁中不帶尾隨空白、英譯自帶（`"At most "`）。兩邊各留一份的結果是同一顆 key 長出兩種繁中，切回繁中時會互相覆蓋。⚠️ **這一種分岔目前沒有網**：「同 key 繁中一致」那條測試收集時 `.trim()`，而 runtime 的 `lang-toggle` 不 trim（它讀的是 `el.textContent`）——差一個半形空白的兩份繁中，測試判成同一個字。要嘛把測試改成不 trim（會先紅一批自帶空白的前後綴，得同批收），要嘛就認清這條靠人審
