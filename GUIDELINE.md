...
- **註解對上游行為的斷言，寫時要對過正本並附出處。出處＝ repo 名 ＋ 符號名（＋ HTTP 端點），三者之外的一律不寫**——特別是**上游的內部模組路徑與檔名一個字都不准出現**（有測試把關）。理由不是簡潔：那一欄是整條引用裡**唯一**會因為對方重構而死掉的部分，而本專案的測試比不到隔壁 repo 的 Python，它壞掉沒有任何一關會紅。實測（上游兩次改組之後）：637 處跨 repo 引用有 149 處指到不存在的檔，其中 **148 處符號還在、值還對**——訊噪比 1:148，而唯一真正過期的那一條被埋在噪音裡。**符號名反過來活得下來**：檔案搬家、函式不改名；改名了就是一個**該被看見**的訊號。同一條的既有推論在低一層已經下過（「不得引行號」——行號會漂到語意相反的那一支），只是把它往上推一層：帶目錄的檔名有一模一樣的性質。
  ...
  - **斷言必須能被一行可貼出的引文支撐**——引的檔案存在但通篇沒有那件事，是最難抓的一種假出處（實測：「100 為後端 Limit 上限」指名了一支確實存在的 controller，而該檔全 51 行沒有任何分頁邏輯）。**跨 repo 查證前先確認對應分支**：gufofaq-saas 的 `master` 未必是當前正本（對應正本在 gufofaq-saas `apps/web/ELEVENTY-SYNC.md` 指到的分支／worktree），查錯分支會把正確的斷言判成錯的。**符號名要真的 grep 得到**——差一個底線（`PROFILE_FIELD_DEFAULTS` 寫成 `_PROFILE_FIELD_DEFAULTS`）是照字面看不出來的假出處。**出處也會過期**：動到該頁時要回頭重讀那幾行，不是只確認檔案還在——正本改版之後，那句話就從「描述現況」變成「描述曾經」。**跨 repo 的「待回流／已回流」清單一律釘對方的 commit sha**（`gufofaq-saas@02ddb25`）：沒有那顆 sha，下一輪只能重讀整份檔案才知道該不該重驗，而這種清單過期得非常快（實測過：沒有 sha 時，清單裡已被對方回流的與還沒回流的看起來一模一樣）。**理由也不得被同一個檔案自己的 markup 推翻**：「不畫 X 是因為這裡沒有 Y」而同檔就有 Y，是比沒寫理由更難抓的一種——它讀起來完整、逐顆有交代。**抄了正本的「值」（指紋、常數、種類數）就要能被機器比對**，比不到就只指名符號、不抄值。
