...
- **`stateItems` 逐條掛 key，逐條可再帶一行成因。** 每一條是 `{textKey, text}` ＋選填的
  `{noteKey, note}`：**結論**（這條規則現在成不成立）是可掃視的一行、也是一組枚舉的成員，**成因**
  （為什麼是這樣）是它下面一行灰字。三件事各自是規則，不是排版偏好——
  **成因不取代結論**（換掉之後那組 key 沒有渲染點，讀的人還要自己從機制推回結論）、
  **成因不收進第二顆 ⓘ**（這顆窗本身就是 ⓘ 開的，窗裡再開窗＝`<dialog>` 疊 `<dialog>`，換到的只是
  幾十 px 高度，而「為什麼」正是讀完結論後最會追問的那一句）、
  **成因與結論同樣走 key**。最後一條是 React 這一側最容易做反的：上游宣告若附了一段現成的說明文字，
  那是**產品打的字**（§4-2 判準：租戶打的才是資料）⇒ 在 i18n 射程內；把它原樣渲染省下的是我們的維護，
  換來的是英文模式下那一行永遠是繁中——而 `fpdiff` 比的是繁中預設快照，抓不到。要真的「與上游同進退」，
  上游得連 key（或兩個語系）一起投影，前端拿 key 查字典，形狀與 `{textKey, text}` 完全相同。
...
- **上游「沒有給這個欄位」是一個獨立的視覺態，不是枚舉的預設值。** 批次匯入 `results[]` 裡 `ok: false`
  ...
  `state || "failed"` 這種收法會把它併進某個真實態——畫面於是宣告了一件沒發生過的事，而那一態的
  動作指示還是**相反的**（「把資料修好、重新匯入」對一個根本沒送出去的檔）。留白也不行：一格空白
  讀起來是版面漏畫，不是「這裡刻意沒有東西」。React 端的形狀：`state?: SyncState`（`undefined` 是
  合法輸入），對照表查不到就走**缺席變體**（`verdict-tag` 最輕的 `.is-faint`），不 fallback 到任何一個
  成員。四態＋缺席態的對照表正本在切版的 `components/import-sync-tag`，React 照抄那條 if 鏈的順序
  （缺席態要判在「認不得的值當 unknown」那條兜底**之前**，否則會被吃掉）。
- **逐筆的狀態不得在 React 端壓成單一 prop。** `results[]` 每一筆各帶一份狀態（一筆一條管道、各自
  成敗）時，元件收的是**陣列**、逐列各畫一顆；彙總那一顆是**另一個衍生值**，由 `results[]` 現算
  （優先序 `unknown` > `failed` > `pending` > `succeeded`，取最安全的那一邊而不是最嚴重的那一邊——
  兩者的指示相反，而「絕對不要重匯」是不會製造重複資料的那一邊），不另存欄位。把 N 份壓成 1 份的
  那一版會讓成功的那幾筆從畫面上消失；**單元測試只釘「彙總算對了」是釘不到這件事的**，要釘的是
  「每一列都有自己那一顆，而且 `ok: false` 那一列不是失敗態」。
...
- **toast 的 `|` 段數是契約**：React 端把段數硬編成 `segs[0..n]`，字典段數一改所有索引位移＝在錯的情況彈錯訊息。
  **真正會安靜錯位的不是「切版加了一段」，是「同一顆 key 兩側段數本來就不同」**：切版往末位追加
  是安全的（前段索引不動），但如果 React 端手上是一份**更短**的同名版本，它的末段型別會落在別的
  索引上——例如兩側都叫 `toast.savePlatformRole`，React 是 2 段（`segs[1]` 是 error）、切版是 3 段
  （error 在 `segs[2]`，`segs[1]` 換成 warning）。這種落差 diff 看不出來（key 名一樣、段數不一樣），
  對接時要逐顆比**段數**，不是比 key 是否存在。說成「切版新增了一段」會讓人只去找新加的呼叫點，而要改的是**每一顆既有呼叫點的索引**。
